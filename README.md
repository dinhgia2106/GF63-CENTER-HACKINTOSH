...
- Cooling, performance and battery-control UI guarded by a read-only safety gate.
...
No Embedded Controller bytes are written in version `0.1.0`. Hardware writes remain disabled until R3EC detects a firmware profile that is explicitly supported and validated for the current GF63 variant.
